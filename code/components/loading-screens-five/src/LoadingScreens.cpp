...
void LoadsThread::DoRun()
{
...
#ifdef IS_RDR3
			// We call this there because it take some to shutdown loading screen even if user call shutdown nui and native loading screen same time native one need to finish animation...
	...
	{
#ifdef IS_RDR3
		// We call this there and wait because it take some to shutdown loading screen
		// SHUTDOWN_LOADING_SCREEN
		fx::ScriptContextBuffer ctx;
		g_origShutdown(ctx);

		// IS_LOADING_SCREEN_VISIBLE
		if (NativeInvoke::Invoke<0xB54ADBE65D528FCB, bool>())
		{
			return;
		}
#endif
